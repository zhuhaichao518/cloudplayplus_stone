...
void FlutterWebRTC::HandleMethodCall(
  ...
  if (method_call.method_name().compare("initialize") == 0) {
   ...
   result->Success();
  } else if (method_call.method_name().compare("reinitialize") == 0) {
    Reinitialize();
    ...
    result->Success();
  } else if (method_call.method_name().compare("mediaStreamTrackSetHint") ==
             0) {
    if (!method_call.arguments()) {
      result->Error("Bad Arguments", "Null constraints arguments received");
      return;
    ...
    const EncodableMap params =
        GetValue<EncodableMap>(*method_call.arguments());
    const std::string track_id = findString(params, "trackId");
    const EncodableValue enable = findEncodableValue(params, "contentHint");
    RTCMediaTrack* track = MediaTrackForId(track_id);
    if (track != nullptr) {
      track->set_enabled(GetValue<bool>(enable));
    }
