...
- (instancetype)initWithTextureRegistry:(id<FlutterTextureRegistry>)registry
  ...
  self = [super init];
  if (self) {
    self.videoProcessingQueue = dispatch_queue_create("com.cloudplayplus.videoprocessing", DISPATCH_QUEUE_SERIAL);
    self.lastFrameTime = [NSDate date];
    //self.last2FrameTime = [NSDate date];
...
- (void)renderFrame:(RTCVideoFrame *)frame {
    dispatch_async(self.videoProcessingQueue, ^{
        ...
        NSTimeInterval timeSinceLastFrame = [currentDate timeIntervalSinceDate:self.lastFrameTime];
        //while (timeSinceLastFrame < 0.01666 ) {  // 16.66毫秒约等于1/60秒
        //    NSTimeInterval delay = 0.01666 - timeSinceLastFrame;
        //    [NSThread sleepForTimeInterval:delay];  // 延迟执行以保持帧率
        //}
        self.lastFrameTime = [NSDate date];  // 更新上一帧的时间
...
//大部分情况下帧就像0，2，32，34，48，50这样 每两帧延迟一点播放
/*#pragma mark - RTCVideoRenderer methods
- (void)renderFrame:(RTCVideoFrame *)frame {
    dispatch_async(self.videoProcessingQueue, ^{
        // 检查帧到来的时间
        NSDate *currentDate = [NSDate date];// 更新上一帧的时间
        NSTimeInterval timeSinceLastFrame = [currentDate timeIntervalSinceDate:self.lastFrameTime];
        //NSTimeInterval timeSinceLast2Frame = [currentDate timeIntervalSinceDate:self.last2FrameTime];
        //self.last2FrameTime = self.lastFrameTime;
        //self.lastFrameTime = [NSDate date];
        
        if (timeSinceLastFrame < 0.15 && self->_isFirstFrameRendered) {  // 16.66毫秒约等于1/60秒
            NSTimeInterval delayInSeconds = 0.01666 - timeSinceLastFrame;
            dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
            
            NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
            [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss.SSS"];
            NSString *currentDateTime = [formatter stringFromDate:[NSDate date]];
            NSLog(@"Current sender time: %@", currentDateTime);
            NSLog(@"Delay in seconds: %f", delayInSeconds);
            
            //if (timeSinceLast2Frame > 0.029) {
            dispatch_after(delayTime, self.videoProcessingQueue, ^{
                [self processFrame:frame];
            });
            //}else{
                //不知道为什么 这一帧来得太快 比前面第二帧快不少，手动调整这个时间。
            //    NSTimeInterval delayInSeconds2 = 0.03333 - timeSinceLast2Frame;
            //    dispatch_time_t delayTime2 = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds2 * NSEC_PER_SEC));
            //    dispatch_after(delayTime2, self.videoProcessingQueue, ^{
            //        [self processFrame:frame];
            //    });
            //}
        } else {
...
- (void)renderFrame:(RTCVideoFrame *)frame {
    dispatch_async(self.videoProcessingQueue, ^{
        ...
        if (timeSinceLastFrame < 0.15) {  // 16.66毫秒约等于1/60秒
            NSTimeInterval delayInSeconds = 0.01666 - timeSinceLastFrame;
            dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
...
#pragma mark - RTCVideoRenderer methods
- (void)renderFrame:(RTCVideoFrame*)frame {
  //Haichao:when playout delay is 0, the time frame arrived is very not stable.
  //We need to cache one frame for flutter renderer. 
  //连续的两帧的 arrive time总是非常接近 不知道为啥！
  NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
  [formatter setDateFormat:@"yyyy-MM-dd HH:mm:ss.SSS"];
  NSString *currentDateTime = [formatter stringFromDate:[NSDate date]];
  NSLog(@"Current time: %@", currentDateTime);
